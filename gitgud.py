...
def get_merge_stats(repo_path):
    ...
    for commit in repo.iter_commits('--all'):
        ...
        if author_name not in user_stats:
            user_stats[author_name] = {'total_commits': 0, 'merge_master_commits': 0}
        ...
        if commit.message.lower().startswith('merge branch '):
            user_stats[author_name]['merge_master_commits'] += 1
...
def main():
    ...
    sorted_user_stats = sorted(user_stats.items(), key=lambda x: (x[1]['merge_master_commits'] / x[1]['total_commits']), reverse=True)

    # Print the results
    print("User\tTotal Commits\tMerge Master Commits\tPercentage of Merge Master Commits\tOffendingCommitNameExample")
    print("-" * 80)
    
    for user, stats in sorted_user_stats:
        total_commits = stats['total_commits']
        merge_master_commits = stats['merge_master_commits']
        percentage = (merge_master_commits / total_commits) * 100 if total_commits > 0 else 0

        print(f"{user}\t{total_commits}\t\t{merge_master_commits}\t\t\t{percentage:.2f}%")
